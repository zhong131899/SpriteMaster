#!/bin/bash
# SpriteMaster - 启动脚本 (前后端分离模式) - Linux
...
find_free_port() {
    local port=$1
    while fuser -s $port/tcp 2>/dev/null || netstat -an 2>/dev/null | grep ":$port.*LISTEN" > /dev/null || ss -ln 2>/dev/null | grep ":$port" > /dev/null; do
        port=$((port + 1))
    done
    echo $port
}
